                ...
                <div class="col-md-4 mb-5">
                    <div class="card h-100">
                        <!-- Sale badge-->
                         ...
                         @endif
                        <!-- Product image-->
                        <img class="card-img-top" id="pizza-image" src="{{ asset('uploads/'.$item->image) }}" alt="..." height="150px" />
                        <!-- Product details-->
    ...
    <div class="col-4 border shadow-sm ps-5 pt-5 pe-5 pb-2 mb-5">
        ...
        <form action="{{ route('user#createContact') }}" method="POST" class="my-4">
            ...
            <input type="email" name="email" value="{{ old('email') }}" class="form-control my-3" placeholder="Email">
                ...
                @endif
            <textarea class="form-control my-3" name="message" class="exampleFormControlTextarea1" rows="3" placeholder="Message">{{ old('message') }}</textarea>
                @if($errors->has('message'))
                <p class="text-danger">{{ $errors->first('message') }}</p>
                @endif
            <button type="submit" class="btn btn-outline-dark float-ene">Send  <i class="fas fa-arrow-right"></i></button>
        </form>
    </div>
</div>
